...
class Seeder02 extends Seeder
{
    ...
    {
        // Seed 02_mahasiswa
        $mahasiswaData = json_decode(File::get(database_path('json/02/mahasiswa.json')), true);
        foreach ($mahasiswaData as $mahasiswa) {
            DB::table('02_mahasiswa')->insert([
                'nim' => $mahasiswa['nim'],
                'nama' => $mahasiswa['nama'],
        ...
        // Seed 02_riwayat_kawe
        $riwayatKaweData = json_decode(File::get(database_path('json/02/riwayat_kawe.json')), true);
        foreach ($riwayatKaweData as $riwayat) {
            DB::table('02_riwayat_kawe')->insert([
                'nim_id' => $riwayat['nim'],
        ...
        // Seed 02_riwayat_matakuliah
        $riwayatMatakuliahData = json_decode(File::get(database_path('json/02/riwayat_matakuliah.json')), true);
        foreach ($riwayatMatakuliahData as $riwayat) {
            ...
            if ($matakuliahExists) {
                DB::table('02_riwayat_matakuliah')->insert([
                    'nim_id' => $riwayat['nim'],
