...
class Seeder03 extends Seeder
{
    ...
    {
        // Seed 03_mahasiswa
        $mahasiswaData = json_decode(File::get(database_path('json/03/mahasiswa.json')), true);
        foreach ($mahasiswaData as $mahasiswa) {
            DB::table('03_mahasiswa')->insert([
                'nim' => $mahasiswa['nim'],
                'nama' => $mahasiswa['nama'],
        ...
        // Seed 03_riwayat_kawe
        $riwayatKaweData = json_decode(File::get(database_path('json/03/riwayat_kawe.json')), true);
        foreach ($riwayatKaweData as $riwayat) {
            DB::table('03_riwayat_kawe')->insert([
                'nim_id' => $riwayat['nim'],
        ...
        // Seed 03_riwayat_matakuliah
        $riwayatMatakuliahData = json_decode(File::get(database_path('json/03/riwayat_matakuliah.json')), true);
        foreach ($riwayatMatakuliahData as $riwayat) {
            ...
            if ($matakuliahExists) {
                DB::table('03_riwayat_matakuliah')->insert([
                    'nim_id' => $riwayat['nim'],
