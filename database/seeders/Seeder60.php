...
class Seeder60 extends Seeder
{
    ...
    {
        // Seed 60_mahasiswa
        $mahasiswaData = json_decode(File::get(database_path('json/60/mahasiswa.json')), true);
        foreach ($mahasiswaData as $mahasiswa) {
            DB::table('60_mahasiswa')->insert([
                'nim' => $mahasiswa['nim'],
                'nama' => $mahasiswa['nama'],
        ...
        // Seed 60_riwayat_kawe
        $riwayatKaweData = json_decode(File::get(database_path('json/60/riwayat_kawe.json')), true);
        foreach ($riwayatKaweData as $riwayat) {
            DB::table('60_riwayat_kawe')->insert([
                'nim_id' => $riwayat['nim'],
        ...
        // Seed 60_riwayat_matakuliah
        $riwayatMatakuliahData = json_decode(File::get(database_path('json/60/riwayat_matakuliah.json')), true);
        foreach ($riwayatMatakuliahData as $riwayat) {
            ...
            if ($matakuliahExists) {
                DB::table('60_riwayat_matakuliah')->insert([
                    'nim_id' => $riwayat['nim'],
